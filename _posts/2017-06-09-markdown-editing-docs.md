---
layout: post
title: "Markdown Editing Docs"
date: 2017-06-09 08:14:47
gambar: "http://orig06.deviantart.net/95ee/f/2013/028/8/4/coming_to_life_by_aoao2-d5t0tuk.jpg"
file: 2017-06-09-markdown-editing-docs.md
---
...
* At the end of a list item, pressing `Enter` will automatically insert the new list item bullet.
    ...
    - If a list item contains a [GFM task][GFM], pressing `Enter` at the end of the line will continue with a new blank task.
...
| `⌘``⇧``K` | `Shift``Win``K` | Creates or pastes the contents of the clipboard as an inline image on selected text.
...
| `⇧``Tab` | `Shift``Tab` | Fold/Unfold current section.
| `^``⇧``Tab` | `Ctrl``Shift``Tab` | Fold all sections under headings of a certain level.
| `⌘``⌥``PageUp` `⌘``⌥``PageDown` | `Ctrl``Alt``Shift``PageUp` `Ctrl``Alt``Shift``PageDown` | Go to the previous/next heading of the same or higher level
| `⌘``⇧``PageUp` `⌘``⇧``PageDown` | `Ctrl``Shift``PageUp` `Ctrl``Shift``PageDown` |  Go to the previous/next heading
